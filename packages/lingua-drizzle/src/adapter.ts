...
> implements StorageAdapter
{
	...
	async getUntranslated(locale: string): Promise<SourceString[]> {
		...
		if (translatedHashes.length > 0) {
			query = query.where(notInArray(sources.hash as Parameters<typeof notInArray>[0], translatedHashes));
		}
	...
	async getSourcesWithTranslations(locales: string[]): Promise<SourceWithStatus[]> {
		...
		const translationMap = new Map<string, Map<string, { text: string; autoGenerated: boolean; reviewed: boolean }>>();
	...
	async deleteSource(hash: string): Promise<void> {
		...
		// Delete source
		await (db.delete as CallableFunction)(this.sources).where(eq(sources.hash as Parameters<typeof eq>[0], hash));
	}
}
