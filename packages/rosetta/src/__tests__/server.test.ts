...
import { getLocale, getTranslationsForClient, runWithI18n, t } from '../server/context';
...
describe('I18n admin methods', () => {
	test('getSourcesWithStatus() returns sources with translation status', async () => {
		...
		const result = await i18n.getSourcesWithStatus(['zh-TW']);
	...
	test('getStats() returns correct statistics', async () => {
		...
		const stats = await i18n.getStats(['zh-TW']);
	...
	test('markAsReviewed() marks translation as reviewed', async () => {
		...
		const sources = await i18n.getSourcesWithStatus(['zh-TW']);
