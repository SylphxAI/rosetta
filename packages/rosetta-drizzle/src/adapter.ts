...
> implements StorageAdapter
{
	...
	async getUntranslated(locale: string): Promise<SourceString[]> {
		...
		const whereCondition =
			translatedHashes.length > 0 ? notInArray(this.sources.hash, translatedHashes) : undefined;
	...
	async markAsReviewed(locale: string, hash: string): Promise<void> {
		await this.db
			...
			.where(and(eq(this.translations.locale, locale), eq(this.translations.hash, hash)));
	}
