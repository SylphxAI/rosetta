...
export class DrizzleStorageAdapter implements StorageAdapter {
	...
	/**
	 * Save multiple translations at once (parallel for better performance)
	 */
	async saveTranslations(
		...
		const results = await Promise.allSettled(
			items.map((item) =>
				this.saveTranslation(locale, item.hash, item.text, {
		...
		for (const result of results) {
			if (result.status === 'rejected') {
				console.error('[rosetta-drizzle] Failed to save translation:', result.reason);
			}
		}
	}
