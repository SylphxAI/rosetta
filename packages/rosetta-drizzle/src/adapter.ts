...
export interface TranslationsTableColumns {
	...
	hash: AnyColumn;
	text: AnyColumn;
	/** Optional: Source text at time of translation (for staleness detection) */
...
> implements StorageAdapter
{
	...
	async saveTranslation(
		...
		// Build values object (conditionally include translatedFrom if column exists)
		...
		// Build update set (conditionally include translatedFrom if column exists)
		...
		// Add translatedFrom if the column exists and value is provided
	...
	async getSourcesWithTranslations(locales: string[]): Promise<SourceWithStatus[]> {
		...
		// Build select fields - include translatedFrom if column exists
		...
		const translationMap = new Map<
			string,
			Map<string, { text: string; autoGenerated: boolean; reviewed: boolean; translatedFrom?: string | null }>
		>();
		...
		return allSources.map((source) => {
			...
			const translationsResult: Record<
				string,
				{ text: string | null; autoGenerated: boolean; reviewed: boolean; translatedFrom?: string | null } | null
			> = {};

			for (const locale of locales) {
				const t = sourceTranslations?.get(locale);
				translationsResult[locale] = t
					? { text: t.text, autoGenerated: t.autoGenerated, reviewed: t.reviewed, translatedFrom: t.translatedFrom }
					: null;
			}
	...
	async saveTranslations(
		...
		// Build values - conditionally include translatedFrom
		...
		// Build update set - conditionally include translatedFrom
		const updateSet: Record<string, unknown> = {
